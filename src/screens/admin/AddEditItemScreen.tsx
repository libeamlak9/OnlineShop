...
export function AddEditItemScreen() {
  ...
  useEffect(() => {
    if (categories.length === 0 && !existing) {
      showAlert(
        ...
        [{ id: 'ok', text: 'OK', onPress: () => navigation.navigate('AdminDashboard') }]
      );
    }
  ...
  async function pickImages() {
    ...
    if (status !== 'granted') {
      await showAlert('Permission needed', 'Please allow access to your photo library.');
      return;
    }
  ...
  async function handleSave() {
    ...
    try {
      if (existing) {
        ...
        await showAlert('Item Updated', `${productData.name} has been updated.`);
      } else {
        await addProduct(productData);
        await showAlert('Item Added', `${productData.name} has been added.`);
      }
      navigation.navigate('AdminDashboard');
    } catch {
      await showAlert('Error', 'Failed to save item. Please check your connection and try again.');
    }
  }
